...
def calculate_polster_errors(state_polls,state_results):
  ...
  pollster_avg = pollster_avg.drop(['Dem_p','Rep_p','Dem_a','Rep_a'], 1)
  pollster_avg = pollster_avg.groupby(['Pollster']).mean().reset_index().drop(['Month'], 1)
  pollster_avg = pollster_avg[['Pollster','PollsterError']]
  return pollster_avg
...
def pred_state_electoral_outcomes(state_preds,ec_2012):
  pred = pd.merge(state_preds,ec_2012 , how='inner', on=['State'])
...
def predict_state_spreads_using_most_recent_polling_data(state_polls,pollster_errors):
  ...
  state_polls["PollsterError"].fillna(5, inplace=True)
...
def main():
  ...
  pollster_errors_2008 = calculate_polster_errors(most_recent_state_polls_2008,state_results_2008)
